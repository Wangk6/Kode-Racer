...
public class LevelOneEpisodeOne {
	...
	public void levelFinishSuccess() {
		...
		win.setLayoutX(225);
		win.setLayoutY(225);
		win.setText("Proceeding to Episode Two");
		...
		displayWin.setOnSucceeded(new EventHandler<WorkerStateEvent>() {
			@Override
			public void handle(WorkerStateEvent event) {
				LevelOneEpisodeTwo two = new LevelOneEpisodeTwo();
				closeProgram();
				two.start();
			}
		});
	...
	private void startedGame() {
		if (startedGame == true) {
			Platform.runLater(() -> new LevelOneEpisodeOne().start());
		}
	}
