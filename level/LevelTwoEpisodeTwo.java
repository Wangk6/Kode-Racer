...
public class LevelTwoEpisodeTwo {
	
	Stage stage = new Stage();
	private Pane root = new Pane();
	
	String backgroundAudio = "src/assets/Level5.mp3";
	...
	private Parent createContent() {
		...
		root.setStyle(
				"-fx-background-image:url('assets/Level2E2.JPG');"
	...
	public void levelFinishSuccess() {
        ...
        displayWin.setOnSucceeded(new EventHandler<WorkerStateEvent>() {
            @Override
            public void handle(WorkerStateEvent event) {
            	LevelTwoEpisodeThree three = new LevelTwoEpisodeThree();
        		closeProgram();
        		three.start();
            }
        });
	...
	private void startedGame() {
		if(startedGame == true) {
			Platform.runLater( () -> new LevelTwoEpisodeTwo() .start());
		}
	}
